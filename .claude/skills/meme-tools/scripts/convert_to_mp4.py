...
def convert_to_mp4(input_file, output_file, quality="medium"):
    ...
    if quality not in QUALITY_PRESETS:
        print(f"Error: Invalid quality. Choose from: {', '.join(QUALITY_PRESETS.keys())}", file=sys.stderr)
        return False

    preset = QUALITY_PRESETS[quality]

    print(f"Converting {os.path.basename(input_file)} to MP4 ({quality})...", file=sys.stderr)
    ...
    cmd = [
        ...
        "-preset", "medium",
        "-b:v", preset["bitrate"],
        "-vf", f"scale=min(iw\\,{preset['scale']}):min(ih\\,{preset['scale']}):force_original_aspect_ratio=decrease",
        "-c:a", "aac",
        "-b:a", "128k",
        "-y",
        output_file
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"✓ Converted: {os.path.basename(output_file)} ({file_size:.1f}MB)", file=sys.stderr)
            return True
...
def main():
    ...
    parser.add_argument("-o", "--output", help="Output MP4 file (default: input_name.mp4)")
